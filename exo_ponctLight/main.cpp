...
int main()
{
    ...
    auto ctx           = p6::Context{{1280, 720, "TP_PointLIGHT"}};
    ctx.maximize_window();
    ...
    ctx.update = [&]() {
        ...
        glUniform3fv(earthProgram.uLightPos_vs, 1, glm::value_ptr(glm::vec3(glm::rotate(ViewMatrix.getViewMatrix(), ctx.time(), glm::vec3(0, 1, 0)) * glm::vec4(1, 1, 0, 1))));
        ...
        for (int i = 0; i < 32; i++)
        {
            ...
            glUniform3fv(moonProgram.uLightPos_vs, 1, glm::value_ptr(glm::vec3(glm::rotate(ViewMatrix.getViewMatrix(), ctx.time(), glm::vec3(0, 1, 0)) * glm::vec4(1, 1, 0, 1))));
